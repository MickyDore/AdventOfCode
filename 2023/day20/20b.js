...
function parsePulses(targetPulse, cycle) {
    ...
    while (pulsesToCheck.length) {
        ...
        if (module.type === flipFlop) {
            ...
            for (let i = 0; i < module.outputs.length; i++) {
                let target = module.outputs[i]
                pulsesToCheck.push({type: module.on ? 'HIGH' : 'LOW', from: pulse.target, target })
            }
        }

        if (module.type === conjunction) {
            if (target === targetPulse && Object.keys(modules[target].inputs).includes(pulse.from) && pulse.type === 'HIGH') {
            ...
            let strength = 'HIGH'
            if (Object.values(module.inputs).every(pulse => pulse === 'HIGH')) strength = 'LOW'
