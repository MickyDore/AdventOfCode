...
const findGears = (str, num, i, j) => {
  for (let k = 0; k < str.length; k++) {
    ...
    if (char === "*") {
      const index = `${i}-${j + k}`;
      gearMap[index] = gearMap[index]
        ? [...gearMap[index], parseInt(num)]
        : [parseInt(num)];
    }
  }
};
  ...
  lines.forEach((line, index) => {
    ...
    for (const match of matches) {
      ...
      let above =
        index > 0
          ? previousLine.substring(
              matchIndex + leftOffset,
              matchIndex + match[0].length + rightOffset
            )
          : "";
      let below =
        index < lines.length - 1
          ...
          : "";
      let left =
        matchIndex > 0 ? match.input.charAt(matchIndex + leftOffset) : "";
      let right =
        matchIndex + match[0].length < line.length
          ? match.input.charAt(matchIndex + match[0].length)
          : "";
      ...
      findGears(
        ...
        match[0],
        index,
        matchIndex + match[0].length + rightOffset - 1
      );
    }
  });
