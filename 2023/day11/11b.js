  ...
  for (let i = 0; i < hashLocations.length - 1; i++) {
    for (let j = i + 1; j < hashLocations.length; j++) {
      ...
      expandedRows.forEach((row) => {
        if (
          row > Math.min(hashLocations[i][0], hashLocations[j][0]) &&
          row < Math.max(hashLocations[j][0], hashLocations[i][0])
        ) {
      ...
      expandedColumns.forEach((column) => {
        if (
          column > Math.min(hashLocations[i][1], hashLocations[j][1]) &&
          column < Math.max(hashLocations[j][1], hashLocations[i][1])
        ) {
      ...
      sum +=
        ...
        yOffset +
        (Math.abs(hashLocations[i][0] - hashLocations[j][0]) + xOffset);
    }
  }
