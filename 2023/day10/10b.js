...
export function solve(input) {
    ...
    let pathPoints = [startingPoint, ...neighbourPoints]
    while (pointsAreDifferent(neighbourPoints[0], neighbourPoints[1])) {
        ...
        pathPoints.push(...neighbourPoints)
    }
    console.log(pathPoints.length)
    console.log(Math.min.apply(null, [...pathPoints.map(arr => arr[0])]))
    console.log(Math.max.apply(null, [...pathPoints.map(arr => arr[0])]))
    return
